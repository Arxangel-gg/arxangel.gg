...
const settings = Object.assign(
  { duration: 60, targetScale: 1, volume: 0.6, xhair: 'cross', xhairColor: '#ffdf82', lbName: '' },
  saveData.settings || {}
);
...
const MODES = {
  gridshot:  { name: 'GRIDSHOT',  desc: 'Three targets up at all times. Clear them fast.',      color: '#ffd76a', r: 42 },
  flick:     { name: 'FLICK',     desc: 'One target, always far away. Snap to it.',             color: '#f5c400', r: 34 },
  precision: { name: 'PRECISION', desc: 'Tiny targets. Zero room for error.',                   color: '#ffeaa6', r: 14 },
  tracking:  { name: 'TRACKING',  desc: 'Keep your crosshair glued to a moving target.',        color: '#e59a4a', r: 46 },
  reflex:    { name: 'REFLEX',    desc: 'Wait for it… then strike instantly.',             color: '#ff8a3d', r: 38 },
};
...
function resize() {
  ...
  // Pin the on-screen (CSS) size to the logical size. Without this the canvas
  // displays at its intrinsic W×dpr size on scaled displays, shifting all
  // visuals away from the true cursor position (worst near the borders).
...
/* Map a pointer event to game coordinates. Scale-aware: exact even if the
   canvas is ever zoomed, transformed, or its CSS size drifts from W×H. */
...
const el = {
  ...
  lbPanel: $('#lbPanel'), lbTabs: $('#lbTabs'), lbList: $('#lbList'), lbDur: $('#lbDur'), lbPlatSeg: $('#lbPlatSeg'),
  resSubmit: $('#resSubmit'), lbName: $('#lbName'), btnPost: $('#btnPost'), resRank: $('#resRank'),
};
...
function finish() {
  ...
  // build results card
  el.resMode.textContent = `${m.name} · ${Math.round(game.duration)}s`;
  ...
  // global leaderboard submit (only when the API is live and there's a score)
  el.resRank.classList.add('hidden');
  el.resRank.textContent = '';
  const canPost = LB.available === true && game.score > 0;
  el.resSubmit.classList.toggle('hidden', !canPost);
  if (canPost) {
    el.lbName.value = settings.lbName || '';
    el.btnPost.disabled = false;
    el.btnPost.textContent = 'POST SCORE';
  }
...
function drawFx() {
  for (const f of game.fx) {
    ...
    if (f.kind === 'dot') {
      ...
      ctx.fillStyle = f.color;
      ctx.font = `700 ${f.size}px Inter, "Segoe UI", sans-serif`;
...
/* Boards are per mode + duration, filterable by platform (all/desktop/mobile).
   Reads hit the table directly (anon key is read-only via RLS). Writes go
   through the submit_arxtrainer_score() RPC, which sanitizes and validates
   server-side — see supabase-leaderboard.sql. If Supabase is unreachable the
   probe fails and the whole feature stays hidden gracefully. */

const PLATFORM = matchMedia('(pointer: coarse)').matches ? 'mobile' : 'desktop';

const LB = {
  url: 'https://glkkdqmeumaxfjlctxuo.supabase.co/rest/v1',
  // Publishable anon key — safe to ship; RLS makes it read-only on this table.
  key: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imdsa2tkcW1ldW1heGZqbGN0eHVvIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTU2OTYyMjEsImV4cCI6MjA3MTI3MjIyMX0.NHKjEDNmgsu5ouKM7vIOJjtNjJM4K2XZA9bjyqf1MK8',
  available: null,          // null = unknown, true/false after probe
  mode: MODE_KEYS[0],
  plat: 'all',              // board filter: all | desktop | mobile
  cache: {},                // "mode_dur_plat" → top[]

  headers(extra = {}) {
    return { apikey: this.key, authorization: `Bearer ${this.key}`, ...extra };
  },

  async fetchTop(mode, dur, plat = 'all', fresh = false) {
    const k = `${mode}_${dur}_${plat}`;
    if (!fresh && this.cache[k]) return this.cache[k];
    const filter = plat === 'all' ? '' : `&platform=eq.${plat}`;
    const r = await fetch(
      `${this.url}/arxtrainerscores?mode=eq.${mode}&dur=eq.${dur}${filter}` +
      `&select=name,score,platform&order=score.desc&limit=10`,
      { headers: this.headers(), cache: 'no-store' }
    );
    if (!r.ok) throw new Error('leaderboard unavailable');
    const data = await r.json();
    this.cache[k] = Array.isArray(data) ? data : [];
    return this.cache[k];
  },

  async init() {
    try {
      await this.fetchTop(this.mode, settings.duration, this.plat);
      this.available = true;
      this.buildTabs();
      this.render();
      el.lbPanel.hidden = false;
    } catch {
  ...
  buildTabs() {
    ...
    MODE_KEYS.forEach(key => {
      ...
      b.style.setProperty('--accent', MODES[key].color);
      b.classList.toggle('on', key === this.mode);
      b.addEventListener('click', () => { this.mode = key; this.buildTabs(); this.render(); });
  ...
  platBadge(p) {
    return p === 'mobile'
      ? '<svg class="lb-plat" viewBox="0 0 24 24" role="img" aria-label="Mobile score">' +
        '<rect x="7" y="2.5" width="10" height="19" rx="2.6" fill="none" stroke="currentColor" stroke-width="1.7"/>' +
        '<circle cx="12" cy="18" r="1.15" fill="currentColor"/></svg>'
      : '';
  },

  async render() {
    if (!this.available) return;
    el.lbDur.textContent = `${settings.duration}s`;
    el.lbList.innerHTML = '<li class="empty">Contacting command…</li>';
    let top;
    try { top = await this.fetchTop(this.mode, settings.duration, this.plat, true); }
    catch { el.lbList.innerHTML = '<li class="empty">Signal lost — try again soon.</li>'; return; }
    if (!top.length) {
      el.lbList.innerHTML = '<li class="empty">No scores yet — claim the top slot.</li>';
      return;
    }
    const mine = (settings.lbName || '').trim().toUpperCase();
    el.lbList.innerHTML = top.map((e, i) =>
      `<li class="${e.name === mine ? 'me' : ''}">` +
      `<span class="lb-rank">#${i + 1}</span>` +
      `<span class="lb-name">${escapeHtml(e.name)}${this.platBadge(e.platform)}</span>` +
      `<span class="lb-score">${Number(e.score) || 0}</span>` +
      `</li>`
    ).join('');
  },

  async post(mode, dur, name, score) {
    const r = await fetch(`${this.url}/rpc/submit_arxtrainer_score`, {
      method: 'POST',
      headers: this.headers({ 'content-type': 'application/json' }),
      body: JSON.stringify({ p_name: name, p_mode: mode, p_dur: dur, p_score: score, p_platform: PLATFORM }),
    });
    if (!r.ok) throw new Error('post failed');
    const data = await r.json();  // { rank, improved, top }
    this.cache = {};              // board changed — drop every cached slice
    if (mode === this.mode && dur === settings.duration) this.render();
    return data;
  },
};
...
async function postScore() {
  ...
  if (name.length < 2) { el.lbName.focus(); return; }
  settings.lbName = name;
  persist();
  el.btnPost.disabled = true;
  el.btnPost.textContent = 'POSTING…';
  try {
    const res = await LB.post(game.mode, game.duration, name, game.score);
    el.resSubmit.classList.add('hidden');
    el.resRank.textContent = res.improved
      ? `★ GLOBAL RANK #${res.rank} ★`
      : `YOUR BEST STANDS — GLOBAL RANK #${res.rank}`;
    el.resRank.classList.remove('hidden');
  } catch {
    el.btnPost.disabled = false;
    el.btnPost.textContent = 'RETRY POST';
  }
}
...
function initSettings() {
  initSeg(el.durSeg, settings.duration, v => {
    ...
    LB.render();      // leaderboard follows the selected duration too
  });
  initSeg(el.lbPlatSeg, 'all', v => {
    LB.plat = v;      // board filter: all | desktop | mobile
    LB.render();
  });
...
/* leaderboard submit — stopPropagation so typing in the callsign field can't
   trigger the global hotkeys (R = restart would eat the letter R!) */
el.btnPost.addEventListener('click', postScore);
el.lbName.addEventListener('keydown', e => {
  e.stopPropagation();
  if (e.key === 'Enter') postScore();
});

// Leaderboard submit — keydown must not bubble to the document handler
// (otherwise typing "r" in the callsign field would restart the run).
el.btnPost.addEventListener('click', postScore);
el.lbName.addEventListener('keydown', e => {
  e.stopPropagation();
  if (e.key === 'Enter') postScore();
...
window.ARXAIM = { startGame, quitToMenu, game, settings, MODES, LB, PLATFORM };
